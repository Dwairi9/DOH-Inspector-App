...
class AttachmentView extends ConsumerWidget {
  late final ChangeNotifierProvider<AttachmentViewProvider> attachmentViewProvider;

  AttachmentView(AttachmentViewProvider attachmentViewProvider, {super.key}) {
    this.attachmentViewProvider = ChangeNotifierProvider<AttachmentViewProvider>((ref) => attachmentViewProvider);
  }

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final provider = ref.watch(attachmentViewProvider);
    final themeNotifier = ref.watch(themeProvider);

    var attachments = provider.attachments;
                ...
                children: [
                  ...
                  ClipOval(
                    child: Material(
                      ...
                      child: Text(attachments.length.toString(), style: Theme.of(context).textTheme.headlineMedium?.copyWith(height: 1.6)),
                    )),
                  ),
                ],
              ),
          ...
          ListView.builder(
              ...
              itemBuilder: (context, index) {
                return Padding(
                  ...
                  child: AttachmentItemWidget(attachment: attachments[index], provider: provider),
                );
              }),
      ],
    );
...
class AttachmentItemWidget extends ConsumerWidget {
  ...
  final AttachmentViewProvider provider;
          ...
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: Row(
                  mainAxisAlignment: provider.isReadOnly ? MainAxisAlignment.center : MainAxisAlignment.spaceBetween,
                  children: [
                    if (!Platform.isIOS)
                      AcamIcon(
                        ...
                        onTap: () {
                          AttachmentRepository.downloadDocument(attachment.documentNo, attachment.fileName, false);
                          ...
                          showDialog(
                              ...
                              builder: (dialogContext) {
                                return AlertDialog(
                                  ...
                                  content: Text("Are you sure you want to delete this attachment?".tr()),
                                  actions: [
                                    TextButton(
                                      onPressed: () {
                                    ...
                                    TextButton(
                                      onPressed: () async {
                                        ...
                                        var actionObject = await provider.deleteAttachment(attachment.documentNo);
...
class ImageFrameWidget extends ConsumerWidget {
  const ImageFrameWidget({
    Key? key,
    required this.attachment,
    required this.attachmentViewProvider,
  }) : super(key: key);

  final Attachment attachment;
  final AttachmentViewProvider attachmentViewProvider;
            ...
            children: [
              if (mimeType.contains("image"))
                InkWell(
                  child: Container(
                    ...
                    child: Utility.getImageFromAccela(attachment.documentNo, userSessionNotifier.getToken(), boxFit: BoxFit.cover),
                  ),
                  onTap: () {
                    openImage(context, attachment.documentNo, attachment.fileName, userSessionNotifier);
                  },
                ),
              if (mimeType.contains("pdf"))
                InkWell(
                  ...
                  onTap: () {
                    openPdf(context, attachment.documentNo, attachment.fileName, attachmentViewProvider);
                  },
                ),
  ...
  openImage(BuildContext context, String documentNo, String title, UserSessionProvider userSessionNotifier) async {
  ...
  openPdf(BuildContext context, String documentNo, String fileName, AttachmentViewProvider provider) async {
...
class AttachmentPdfViewerWidget extends ConsumerWidget {
  const AttachmentPdfViewerWidget({Key? key, required this.attachmentViewProvider}) : super(key: key);

  final AttachmentViewProvider attachmentViewProvider;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final provider = ref.watch(ChangeNotifierProvider((ref) => attachmentViewProvider));
